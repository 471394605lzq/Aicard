...
namespace AiCard
{
    ...
    {
        SmsResult SendSMS(string phone, string code,string content);
    }
            ...
            try
            {
                ...
                using (HttpWebResponse response = request.GetResponse() as HttpWebResponse)
                {
                    ...
                    if (responseStr != null && responseStr.Length > 0)
                    {
                        ...
                        var t = JsonConvert.DeserializeObject<JObject>(responseStr);
                        string tempstr = t["resp"]["respCode"].ToString();
                        //{ "resp":{ "respCode":"000000","failure":"0","templateSMS":{ "createDate":"20181128171805","smsId":"72fa07dbef30d8b3088da2b4ed0d725c"} } }
                        return new SmsResult { IsSuccess = tempstr == "000000" ? true:false, Message = tempstr };
                        //return responseStr;
                    }
                }
                return null;
