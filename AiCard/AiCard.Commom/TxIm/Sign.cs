...
namespace AiCard.Common.TxIm
{
    public class SigCheck
    {
        ...
        {
            var f = new FileStream(ImConfig.PrivateKeyPath, FileMode.Open, FileAccess.Read);
            ...
            int ret = tls_gen_sig_ex(
                Convert.ToUInt32( ImConfig.SdkAppId),
                user,
                sig,
                4096,
                pri_key,
        ...
        {
            // 校验 sig
            var f = new FileStream(ImConfig.PublicKeyPath, FileMode.Open, FileAccess.Read);
            var reader = new BinaryReader(f);
